...
def generate_image(seed, width, height, mean, std):
    ...
    """
    np.random.seed(seed)  # Define a semente para garantir reprodutibilidade
    image = np.random.normal(mean, std, size=(height, width))  # Gera a imagem com distribuição normal
    return image
